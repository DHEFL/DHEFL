...
if __name__ == '__main__':
    args = get_args()
    num = -1
    dataset = pd.read_csv(args.datadir)
    test_data = pd.read_csv('MNIST/test.csv')

    #Normalization 
    dataset.loc[:, 'pixel1':] = dataset.loc[:, 'pixel1':].astype('float32')
    test_data.loc[:, 'pixel1':] = test_data.loc[:, 'pixel1':].astype('float32')

    dataset.loc[:, 'pixel1':] /= 255.0
    test_data.loc[:, 'pixel1':] /= 255.0


    pd.DataFrame(test_data).to_csv('MNIST/scaledtest.csv', index=False) 
    ...
    for i in range(args.n_parties):
        file_name = args.outputdir+'client'+str(i)+'.csv'
